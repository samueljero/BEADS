...
mininet_config_path = os.path.abspath(
    os.path.join(system_home, 'mininet_scripts'))
...
class SDNTester:
    ...
    def __init__(self, mininet, controllers, log):
        assert(isinstance(mininet, (list, tuple)) and isinstance(
            controllers, (list, tuple)) and len(mininet) == 1)
    ...
    def _build_stat_obj(self, config_key_name):
        ...
        return ProcMonStat(multipliers=mx, alg=config.stat_baseline_alg, rebase_threshold=config.stat_rebase_threshold)
    ...
    def baseline(self, test_script):
        ...
        while i < config.stat_baseline_nrounds:
            ...
            res = self.doTest(
                {'switch': None, 'host': None, 'topo': test_script})
            if res[0] == False:
                print "Warning!!! Baseline failed!!!"
                i += 1
            else:
                # Occasionally switch stat cannot capture the stat. When this
                # happens, discard current baseline round and redo baseline.
                try:
                    if hasattr(self, 'switch_stat_dict'):
                        self.switch_stat.add_baseline(
                            stat_dict=self.switch_stat_dict)
                    if hasattr(self, 'controller_stat_dict'):
                        self.controller_stat.add_baseline(
                            stat_dict=self.controller_stat_dict)
                    i += 1
                except ValueError as e:
                    # Simply print the error, and redo the baseline.
                    print e
        
        # After calculating baseline, the history data is cleared.
        self.switch_stat.calc_baseline()
        self.controller_stat.calc_baseline()

        # Process Results
        if config.veriflow_enabled:
            self.veriflow_flips_threshold = (sum(
                self.veriflow_flips) / len(self.veriflow_flips)) * 2  # VeriFlow Flip Threshold
        self.testnum = num
        self.creating_baseline = False
    ...
    def doTest(self, strategy):
        ...
        for c in self.controllers:
            controlleraddrs.append(mv.vm2ip(c) + ":" +
        ...
        if config.veriflow_enabled == True:
            ...
            proxyaddrs = list()
            proxyaddrs.append((config.proxy_addr, vf_port,
                               confg.proxy_base_port + 1))
            if veriflow is None:
                self._stop_controllers()
                proxy.terminate()
                return(False, "System Failure", 0)
        ...
        # Do Test
        test_std_err = None
        res, test_std_err = self._call_test(
            test_script, strategy['host'], proxyaddrs)
        if res is None:
            self._stop_controllers()
        ...
        ctl_perf = 'Controller stat: ' + repr(self.controller_stat_dict) if self.creating_baseline else self._validate_stat(
    ...
    def _validate_stat(self, name, result, statmgr, stat_key):
        if hasattr(self, stat_key):
            is_valid, err_msg, suggest_rebase = self._eval_stat(
                statmgr, getattr(self, stat_key))
            # Add values of both controller's and switch's.
            if not is_valid and result[0]:
                result[0] = False
                result[1] = '%s stat: ' % name + err_msg
                # Suggest rebaseline iff it's performance failure.
                if len(result) == 2:
                    result.append(suggest_rebase)
                else:
                    result[2] += suggest_rebase
            return err_msg
        return ''
    ...
    def startVms(self):
        ...
        for c in self.controllers:
            if(self._waitListening(mv.vm2ip(c), 22, 240, True) == False):
                ...
                proc = shell.run(
                    ["/bin/bash", "-i", "-c", "cd monitors && make clean && make"])
        ...
        for m in self.mininet:
            if(self._waitListening(mv.vm2ip(m), 22, 240, True) == False):
                ...
                if config.mininet_replace_scripts:
                    ...
                    shell = spur.SshShell(hostname=mv.vm2ip(m), username=config.mininet_user,
                                          missing_host_key=spur.ssh.MissingHostKey.accept, private_key_file=config.vm_ssh_key)
                    proc = shell.run(
                        ["/bin/bash", "-i", "-c", "cd monitors && make clean && make"])
                    if proc.return_code is not 0:
    ...
    def _start_proxy(self, controlleraddrs, proxyaddrs):
        proxy = None
        ts = time.time()
        cmd = config.proxy_path + " -p " + \
            str(config.proxy_com_port + self.mininet[0])
        for c in range(0, len(controlleraddrs)):
            cmd = cmd + " -c " + \
                str(proxyaddrs[c][1]) + ":" + controlleraddrs[c]
        self.log.write("Proxy CMD: " + cmd + "\n")
        self.log.write("********* Proxy output ********\n")
        self.log.flush()
        try:
            proxy = subprocess.Popen(
                cmd, shell=True, stdout=self.log, stderr=subprocess.STDOUT)
    ...
    def _start_veriflow(self, test_script, proxyaddrs, vf_port):
        ...
        veriflow = None
        ts = time.time()
        topo_file = config.veriflow_topo_path + os.path.splitext(os.path.basename(
            test_script.format(topo_delay='', controllers="").strip()))[0] + ".vft"
        log_file = config.veriflow_log_path + \
            config.veriflow_log_name.format(instance=self.mininet[0])
        cmd = config.veriflow_path + " " + \
            str(vf_port) + " 127.0.0.1  " + \
            str(proxyaddrs[0][1]) + " " + topo_file + " " + log_file
        self.log.write("Veriflow CMD: " + cmd + "\n")
        self.log.flush()
        try:
            veriflow = subprocess.Popen(
                cmd, shell=True, stdout=self.log, stderr=subprocess.STDOUT)
        except Exception as e:
            print e
            self.log.write("Exception: " + str(e) + "\n")
    ...
    def _start_controllers(self):
        ...
        for c in self.controllers:
            shell = spur.SshShell(hostname=mv.vm2ip(c), username=config.controller_user,
                                  missing_host_key=spur.ssh.MissingHostKey.accept, private_key_file=config.vm_ssh_key)
            res = shell.run(
                ["/bin/bash", "-i", "-c", "~/monitors/control.sh {0} {1}".format(config.controller_type, "start")])
            self.log.write("Starting Controller (" +
                           mv.vm2ip(c) + ")...\n" + res.output)
            self.log.flush()
        for c in self.controllers:
            if(self._waitListening(mv.vm2ip(c), config.controller_port, 60) == False):
                self.log.write(
                    "Controller %s failed to start after %d seconds" % (mv.vm2ip(c), 60))
                self.log.flush()
                return False
            else:
                # Controller starts.
                res = shell.run(
                    ["/bin/bash", "-i", "-c", "~/monitors/control.sh {0} {1}".format(config.controller_type, "mon")])
                self.log.write(
                    "Starting resource monitor for controller (" + mv.vm2ip(c) + ")...\n" + res.output)
                self.log.flush()
        if config.enable_stat:
            self.log.write(
    ...
    def _stop_controllers(self):
        ...
        for c in self.controllers:
            ...
            try:
                self.log.write(
                    "Stopping controller (" + mv.vm2ip(c) + ")...\n")
                res = shell.run(["/bin/bash", "-i", "-c", "~/monitors/control.sh {0} {1}".format(
                    config.controller_type, "stop")], allow_error=True)
                try:
                    ...
                    import traceback
                    self.log.write(
                        'Could not extract controller procmon stat: ' + str(e) + '.\n')
    ...
    def _cleanup(self):
        ts = time.time()
        shell = spur.SshShell(hostname=mv.vm2ip(self.mininet[
                              0]), username=config.mininet_user, missing_host_key=spur.ssh.MissingHostKey.accept, private_key_file=config.vm_ssh_key)
    ...
    def _send_proxy_strategy(self, strategy, proxyaddrs):
        ...
        for l in strategy:
            ...
            res = self._proxy_communicate(
                ("localhost", config.proxy_com_port + self.mininet[0]), cmd)
            if (res == False):
    ...
    def _get_msg_types(self, addr):
        ...
        if type(resp) is bool and resp == False:
            ...
            self.log.write(
                "Error: Failed to get message type feedback from proxy!\n")
            self.log.flush()
            return False
